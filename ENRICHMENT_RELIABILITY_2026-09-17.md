...
| Paid access | Verified owner results are saved separately from user access. Access is granted after a confirmed payment or complimentary authorization. Starting a permit lookup no longer implicitly unlocks its result. Bulk export charges only after successful lookup. |
