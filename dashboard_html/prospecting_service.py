...
Every lookup is scoped to the uploader (team admins can see their team's lists).
...
def scope(ctx):
    ...
    return sql if ctx['is_admin'] else sql + ' AND l.added_by_id = %(user_id)s'
...
def create_list(ctx, parsed, *, name, filename, mapping, import_key):
    ...
    digest = hashlib.sha256(json.dumps([name, parsed['columns'], parsed['rows'], mapping], sort_keys=True).encode()).hexdigest()
    with transaction() as cur:
        cur.execute("""INSERT INTO prospect_lists (name,filename,columns,mapping,import_key,import_hash,team_id,added_by_id)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (added_by_id,import_key) DO NOTHING RETURNING id""",
            (name, filename[:255], Json(parsed['columns']), Json(mapping), import_key, digest, ctx['team_id'], ctx['user_id']))
        row = cur.fetchone()
        if not row:
...
def get_list(ctx, list_id, cur):
    cur.execute(f'SELECT l.* FROM prospect_lists l WHERE l.id=%(id)s AND {scope(ctx)}', {**ctx, 'id': list_id})
    result = cur.fetchone()
    if not result:
...
def list_lists(ctx):
    with transaction() as cur:
        cur.execute(f"""SELECT l.id,l.name,l.filename,l.created_at,COUNT(r.id) AS row_count,
...
def get_row(ctx, row_id, cur, *, lock=False):
    cur.execute(f"""SELECT r.* FROM prospect_rows r JOIN prospect_lists l ON l.id=r.list_id
        WHERE r.id=%(id)s AND {scope(ctx)} {'FOR UPDATE OF r' if lock else ''}""", {**ctx, 'id': row_id})
    row = cur.fetchone()
    if not row:
    ...
    with transaction() as cur:
        ...
        if existing:
            ...
            cur.execute("""INSERT INTO crm_contacts(name,title,company,email,source,source_detail,assigned_to_id,added_by_id,team_id)
                ...
                (fields['name'], fields['title'] or None, fields['company'] or None, fields['email'] or None,
                 source, listing['added_by_id'], ctx['user_id'], ctx['team_id']))
        ...
        if row['next_follow_up']:
            cur.execute("""INSERT INTO crm_follow_ups(title,note,due_date,contact_id,assigned_to_id,created_by_id,team_id)
                ...
                contact_id, (existing['assigned_to_id'] or listing['added_by_id']) if existing else listing['added_by_id'], ctx['user_id'], ctx['team_id']))
